...
import { Check, ArrowRight, ChevronDown, ChevronUp, AlertTriangle, ShieldCheck, Leaf, Zap, Brain, X, Info, Activity, Share2, Download } from 'lucide-react';
// @ts-ignore
...
export const ResultView: React.FC<{ result: ScanResult; onBack: () => void; onScanNew: () => void }> = ({ result, onBack, onScanNew }) => {
  ...
  const criticalRisks = useMemo(() => {
    return result.additives?.filter(a => a.riskLevel === 'high' || a.riskLevel === 'medium').slice(0, 3) || [];
  }, [result.additives]);
        ...
        >
          ...
          {activeTab === 'ingredients' && (
            <div className="space-y-4">
              ...
              {result.additives?.length ? result.additives.map((item, i) => (
                <motion.div 
                  key={i} 
                  layout
